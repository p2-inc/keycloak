...
public class FlowTest extends AbstractAuthenticationTest {
    ...
    public void testAddRemoveFlow() {
        ...
        AuthenticationFlowRepresentation builtInFlow = flows.stream()
                .filter(AuthenticationFlowRepresentation::isBuiltIn).findAny().orElse(null);
        ...
        try {
            Assertions.assertEquals(409, response.getStatus(),
                    "createFlow using the alias of existing flow should fail");
        } finally {
            response.close();
        }
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                AdminEventPaths.authAddExecutionFlowPath("browser-2"), data, ResourceType.AUTH_EXECUTION_FLOW);
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                AdminEventPaths.authAddExecutionFlowPath("browser-2"), data2, ResourceType.AUTH_EXECUTION_FLOW);
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.DELETE,
                AdminEventPaths.authFlowPath(found.getId()), ResourceType.AUTH_FLOW);
    ...
    public void testRemoveUsedFlow() {
...

        {
            ...
            BiConsumer<Supplier<String>, Consumer<String>> assertRemoveFailInRealm = (rollbackFlow, updateFlow) -> {
...

        {
            ...
            BiConsumer<Supplier<String>, Consumer<String>> assertRemoveFailByIdp = (rollbackIdp, updateIdp) -> {
    ...
    public void testCopyFlow() {
        ...
        try {
            assertThat("Copy flow using the new alias of existing flow should fail", response,
                    statusCodeIs(Status.CONFLICT));
            assertThat("Copy flow using the new alias of existing flow should fail", response,
                    body(containsString("already exists")));
            assertThat("Copy flow using the new alias of existing flow should fail", response,
                    body(containsString("flow alias")));
        } finally {
            response.close();
        }
        ...
        // compare original flow with a copy - fields should be the same except id,
        // alias, and builtIn
        List<AuthenticationFlowRepresentation> flows = authMgmtResource.getFlows();
    ...
    public void addExecutionFlow() {
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                AdminEventPaths.authAddExecutionFlowPath("parent"), params, ResourceType.AUTH_EXECUTION_FLOW);
    ...
    public void editFlowTest() {
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.UPDATE,
                AdminEventPaths.authEditFlowPath(testFlow.getId()), ResourceType.AUTH_FLOW);
        flows = authMgmtResource.getFlows();
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.UPDATE,
                AdminEventPaths.authEditFlowPath(found.getId()), ResourceType.AUTH_FLOW);
    ...
    public void editExecutionFlowTest() {
        ...
        AuthenticationFlowRepresentation newFlow = newFlow("Parent-Flow", "This is a parent flow", "basic-flow", true,
                false);
        createFlow(newFlow);
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                AdminEventPaths.authAddExecutionFlowPath("Parent-Flow"), params, ResourceType.AUTH_EXECUTION_FLOW);
        ...
        authMgmtResource.updateExecutions("Parent-Flow", found);
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.UPDATE,
                AdminEventPaths.authUpdateExecutionPath("Parent-Flow"), ResourceType.AUTH_EXECUTION);
        executionReps = authMgmtResource.getExecutions("Parent-Flow");
    ...
    public void failWithLongDescription() throws IOException {
        ...
        try {
            ...
            try (Response response = isee.getResponse()) {
                ...
                assertFalse(StreamUtil.readString((InputStream) response.getEntity(), StandardCharsets.UTF_8)
    ...
    public void testAddRemoveExecutionsFailInBuiltinFlow() throws IOException {
        // get a built in flow
        List<AuthenticationFlowRepresentation> flows = authMgmtResource.getFlows();
        AuthenticationFlowRepresentation flow = flows.stream().filter(AuthenticationFlowRepresentation::isBuiltIn)
        ...
        BadRequestException e = Assertions.assertThrows(BadRequestException.class,
                () -> authMgmtResource.addExecution(flow.getAlias(), data));
        OAuth2ErrorRepresentation error = e.getResponse().readEntity(OAuth2ErrorRepresentation.class);
        ...
        e = Assertions.assertThrows(BadRequestException.class,
                () -> authMgmtResource.removeExecution(executions.get(0).getId()));
        error = e.getResponse().readEntity(OAuth2ErrorRepresentation.class);
    ...
    public void testExecutionConfigDuplicated() {
        ...
        List<AuthenticationExecutionInfoRepresentation> executions = authMgmtResource
                .getExecutions(existingFlow.getAlias());
        ...
        try (Response response = authMgmtResource.newExecutionConfig(executionWithConfig.getId(), executionConfig)) {
            ...
            AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                    AdminEventPaths.authAddExecutionConfigPath(executionWithConfig.getId()), executionConfig,
                    ResourceType.AUTHENTICATOR_CONFIG);
        }
        ...
        AdminEventAssertion.assertEvent(adminEvents.poll(), OperationType.CREATE,
                AdminEventPaths.authCopyFlowPath("browser"), copyFlowParams, ResourceType.AUTH_FLOW);
        ...
        for (String executionConfigId : newExecutionConfigIds) {
            Assertions.assertFalse(existingExecutionConfigIds.contains(executionConfigId),
                    "Execution config not duplicated");
        }
    }
}
