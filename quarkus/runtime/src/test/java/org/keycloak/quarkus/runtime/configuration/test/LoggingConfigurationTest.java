...
public class LoggingConfigurationTest extends AbstractConfigurationTest {
    ...
    public void syslogDefaults() {
        ...
        assertThat(Configuration.getOptionalKcValue(LoggingOptions.LOG_SYSLOG_MAX_LENGTH).orElse(null),
        ...
        // The default max-length attribute is set in the
        // org.jboss.logmanager.handlers.SyslogHandler if not specified in config
        assertThat(Configuration.getOptionalValue("quarkus.log.syslog.max-length").orElse(null),
                CoreMatchers.nullValue());
    }
    ...
    public void testWildcardEnvVarOptionCanBeMappedToQuarkusOption() {
        ...
        Set<String> keys = StreamSupport.stream(config.getPropertyNames().spliterator(), false)
